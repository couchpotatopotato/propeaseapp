<template>
    <table id="table">
        ...
        <tr v-for="row in tableRows" :key="row.propAddress">
            <td v-if="row.tenantEmail === useremail && (row.receiver === 'Tenant' || row.receiver === 'Both')">{{ row.propAddress }}</td>
            <td v-if="row.tenantEmail === useremail && (row.receiver === 'Tenant' || row.receiver === 'Both')">{{ row.ownerName }}</td>
            ...
            async fetchAndUpdateData(useremail) {
                ...
                this.tableRows = await Promise.all(
                    allDocuments.docs.map(async (documents) => {
                        ...
                        const ownerDetails = await getDoc(ownerRef);
                        let ownerName = ownerDetails.data().Name;
